...
<body>
    ...
    <!-- edit pesanan -->
    <form method="post" action="proses/update_order.php">
        <section class="container">
            <h3 class="mt-5">Edit Status Pesanan</h3>
            <div class="mb-3">
                <label for="descProduct" class="form-label">Status Pesanan</label>
                <select class="form-control" name="status_order">
                    <option <?= ($tampil['status_order'] == "") ? 'selected' : '' ?> disabled>Ubah Status Pesanan
                    </option>
                    <option value="belum dibayar" <?= ($tampil['status_order'] == "belum dibayar") ? 'selected' : '' ?>>
                        Belum Dibayar</option>
                    <option value="dibayar" <?= ($tampil['status_order'] == "dibayar") ? 'selected' : '' ?>>Dibayar
                    </option>
                    <option value="dibatalkan" <?= ($tampil['status_order'] == "dibatalkan") ? 'selected' : '' ?>>
                        Dibatalkan</option>
                    <option value="dalam proses" <?= ($tampil['status_order'] == "dalam proses") ? 'selected' : '' ?>>
                        Dalam Proses</option>
                    <option value="dalam pengiriman"
                        <?= ($tampil['status_order'] == "dalam pengiriman") ? 'selected' : '' ?>>Dalam Pengiriman
                    </option>
                    <option value="terkirim" <?= ($tampil['status_order'] == "terkirim") ? 'selected' : '' ?>>Terkirim
                    </option>

                    <input type="hidden" name="tipe" value="seller">
                    <input type="hidden" name="id_order" value="<?php echo $id_order; ?>">
            </div>
            <div class="mb-5">
                <button type="submit" class="btn btn-primary mt-5">Perbarui Status Pesanan</button>
            </div>
        </section>
    </form>
