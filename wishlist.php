...
if(isset($_POST['delete'])){
    ...
    for($i=0;$i<count($checkbox);$i++){
        $del_id = $checkbox[$i]; 
        mysqli_query($koneksi, "DELETE FROM wishlist WHERE id_wish='".$del_id."'");
    }
...
<body>
    ...
    <form action="" method="POST">
        <div class="container">
            ...
            <table class="table">
                ...
                <tbody>
                    <?php
                $sql="SELECT * FROM wishlist JOIN akun ON wishlist.id_akun = akun.id_akun JOIN barang ON wishlist.id_barang = barang.id_barang JOIN toko ON barang.id_toko = toko.id_toko WHERE akun.username='$_SESSION[username]' OR akun.email='$_SESSION[username]' OR akun.no_telp='$_SESSION[username]'";
                $data_tabel = mysqli_query($koneksi, $sql);
                $total = mysqli_num_rows($data_tabel);
                ...
                echo "<tr>
                        ...
                        <input type='checkbox' id='checkItem' name='check[]' value=".$data['id_wish'].">
                        </td>
                    ...
                    <td>
                        <form method='POST' action='proses/add_order.php'>
                            <input type='hidden' name='tipe' value='wish'>
                            ...
                            <input type='hidden' name='status_bayar' value='belum dibayar' />
                            <button type='submit' class='btn btn-primary btn-icon btn-md'><i
                                    class='fa fa-shopping-cart'></i></button>
                        </form>
                    </td>
                    <td>

                        <a href='proses/del_wish_cart.php?id_wish=".$data['id_wish']."&tipe=wish'
                            type='button' class='btn btn-danger btn-icon btn-md'>
        ...
        <section class="pt-5 pb-5">
            <div class="container">
                <div class="row">
                    <?php
                    if ($total_wish==0) {
                      echo "<button type='button' class='btn btn-danger' disabled hidden>Hapus Semua</button>";
                    }else {
                        echo "<button type='submit' class='btn btn-danger' name='delete'>Hapus Semua</button>";
                    }
                    ?>
                </div>
            </div>
    ...
    <!-- Modal update cart-->
    <div class="modal fade" id="updateCartModal" tabindex="-1" role="dialog" aria-labelledby="updateCartLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="updateCartLabel">Konfirmasi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">Apakah anda yakin untuk mengubah isi keranjang ?</div>
                <div class="modal-footer">
